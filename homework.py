...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        return ((self.COEFF_RUNNING_MULT * self.get_mean_speed()
                 - self.COEFF_RUNNING_SUBTR) * self.weight / self.M_IN_KM
                * self.duration * self.MIN_IN_HOUR)


class SportsWalking(Training):
    """Тренировка: спортивная ходьба."""
    COEFF_WALKING_ADD: float = 0.035
    COEFF_WALKING_MULT: float = 0.029
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        return ((self.get_mean_speed() + self.COEFF_SWIMMING_ADD)
                * self.COEFF_SWIMMING_MULT * self.weight)
